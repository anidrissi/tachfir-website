# PROMPT MAÎTRE — Refonte tachfir.com · Next.js trilingue (AR par défaut / FR / EN) · CMS intégré · Git & CI/CD · SEO Maroc + Golfe + francophonie
...
> 5. Le build local fonctionne sans aucun compte externe (SQLite). Pour la mise en ligne : GitHub + Vercel + une base Postgres (Neon ou Vercel Postgres) — le README généré vous guidera pas à pas.

---

## 1. Rôle et mission

Tu es un développeur senior Next.js doublé d'un expert SEO international (marchés arabophone et francophone). Ta mission : construire **de zéro** le nouveau site de **Tachfir** (`https://www.tachfir.com`), production-ready, trilingue **arabe (langue par défaut), français, anglais**, avec **CMS intégré** pour que l'équipe gère elle-même le blog, les formations, les expertises et les contenus clés, le tout **versionné sous Git avec pipeline CI/CD**. Objectif business : générer des leads — devis, bons de commande, missions d'outsourcing et de placement de talents IT (Maroc, **France, Belgique, Canada, pays du Golfe**), inscriptions formations — et ranker sur Google en arabe, français et anglais.
...
- **4 pôles d'activité** :
  ...
  3. **Outsourcing & Talents IT** — prestation de services (régie / staff augmentation), offshoring-nearshoring, TMA & centre de services, et **chasse, qualification et placement de profils IT** (talent hunting). Profils phares : **Java/Spring, Angular/React, ServiceNow, Pentesting, DevOps & Cloud**. Marchés servis : Maroc · **France, Belgique, Canada** (francophonie) · **pays du Golfe** (EAU, Arabie Saoudite, Qatar — en arabe/anglais).
  4. **Formation** — formations IT certifiantes (nouveau pôle à mettre en avant)
  - **+ Fourniture de matériel informatique et de surveillance** — vente **sur demande, sans catalogue en ligne** : l'entreprise répond aux bons de commande, appels d'offres (طلبات العروض) et demandes de prix des administrations publiques et sociétés marocaines. Inclut la **vidéosurveillance et le contrôle d'accès** (معدات المراقبة).
...
- **Cibles** : DSI et acheteurs publics marocains (marchés publics, bons de commande, appels d'offres) · PME/entreprises marocaines · ESN, ETI et PME en France, Belgique et Canada (prestation, outsourcing, talents) · entreprises et intégrateurs des pays du Golfe (contenus AR/EN) · professionnels et entreprises pour la formation.
...
5. Polices via `next/font/google`, `display: swap` :
   ...
   - Vérifie l'harmonie AR/latin (hauteurs, graisses) quand les deux cohabitent (chiffres, noms de technologies comme Java, Angular, ServiceNow qui restent en latin dans le texte arabe).
6. Sélecteur de langue dans le header : conserve la page courante lors du changement (mapping via pathnames **et via les slugs localisés du CMS** pour les articles/formations/expertises), avec attributs `hreflang` sur les liens.
7. **Arabe = arabe standard moderne professionnel (fusha), avec DEUX registres selon l'audience** :
   - **Pages marchés publics / fourniture / contact Maroc** → terminologie marocaine officielle : المعلوميات (informatique), سند الطلب (bon de commande), عرض أثمان (devis), الصفقات العمومية (marchés publics), **طلبات العروض / طلب عروض** (appels d'offres).
   - **Pages outsourcing / expertises / talents (audience Golfe)** → MSA orienté Golfe : تقنية المعلومات، التعهيد (outsourcing)، استقطاب الكفاءات، توظيف المطورين، المناقصات.
   - Jamais de traduction mot à mot depuis le français.
...
| `/services/outsourcing-talents-it` | Prestation de services (régie/staff augmentation), offshoring-nearshoring, TMA & centre de services, **talent hunting** (sourcing → qualification → placement) ; processus « brief → shortlist sous 72 h → entretiens → démarrage » ; arguments par marché (voir §9) ; liens vers les expertises ; FAQ |
| `/services/fourniture-informatique` | **Page SANS catalogue** — voir spec dédiée §9 (inclut vidéosurveillance & contrôle d'accès) |
| `/expertises` + `/expertises/[slug]` | Hub + **landing pages SEO par technologie, pilotées par le CMS** — 5 seedées : Java/Spring, Angular/React, ServiceNow, Pentesting, DevOps & Cloud. Chaque landing : pitch, profils & séniorités disponibles, modalités (régie / forfait / placement), cas d'usage, CTA « Recevoir des profils sous 72 h » |
| `/talents` | Espace consultants : argumentaire (missions Maroc/France/Golfe, remote), **formulaire de dépôt de CV** → vivier dans l'admin (§8, §10) |
...
Navigation : `Accueil · Services ▾ (Développement · Cybersécurité · Outsourcing & Talents · Fourniture IT · Expertises) · Formations · Références · Blog · Contact` + bouton CTA permanent **« Devis / Bon de commande »** + sélecteur langue + toggle thème. Lien « Consultants : rejoignez notre vivier » (→ `/talents`) dans le footer et sur la page outsourcing. Fil d'Ariane visible sur toutes les pages internes (+ schema BreadcrumbList). Footer riche : NAP complet (depuis le global `settings` du CMS), liens par pôle + expertises, certifications (mini), LinkedIn, mentions légales.
...
|---|---|---|---|
| Accueil | شركة معلوميات بالمغرب · شركة برمجيات | société de services informatiques au Maroc (ESN) | IT services company in Morocco |
| Dév. | تطوير المواقع والتطبيقات · شركة برمجيات بالمغرب | développement web et mobile Maroc | software development company Morocco |
| Cybersécurité | الأمن السيبراني · شركة أمن معلومات | cybersécurité et test d'intrusion Maroc | cybersecurity & pentesting Morocco |
| Outsourcing & Talents | التعهيد التقني وتوفير الكفاءات | outsourcing IT, régie et recrutement de talents IT | IT outsourcing & staff augmentation |
| Expertises | توظيف مطوري Java / Angular · خبراء ServiceNow · اختبار الاختراق | consultant Java, développeur Angular, consultant ServiceNow, expert pentest | hire Java / Angular / ServiceNow / pentesting experts |
| Formations | تكوين في المعلوميات بالمغرب | formation informatique certifiante au Maroc | certified IT training Morocco |
| Fourniture | توريد العتاد المعلوماتي · **معدات المراقبة** | fourniture de matériel informatique et vidéosurveillance au Maroc | IT & surveillance equipment supply Morocco |

Secondaires à disséminer (pages, FAQ, blog) —
**AR** : **طلبات العروض** المعلوماتية، سند طلب معدات معلوماتية، **معدات المراقبة** والكاميرات، أنظمة المراقبة بالفيديو، **شركة أمن معلومات**، **شركة برمجيات**، الصفقات العمومية، دورات تكوينية في البرمجة، التعهيد، توظيف مطورين، المناقصات التقنية ·
**FR** : bon de commande informatique, appel d'offres informatique Maroc, fournisseur matériel informatique administrations publiques, matériel de vidéosurveillance, caméras de surveillance, contrôle d'accès, régie informatique, staff augmentation, recrutement développeurs Java/Angular, consultant ServiceNow francophone, ESN nearshore, externalisation informatique France-Maroc, création site web `[VILLE]` ·
**EN** : IT outsourcing company, staff augmentation Morocco, hire offshore Java/Angular developers, ServiceNow consultants, penetration testing services, CCTV & surveillance equipment supply, tech talent sourcing GCC.

**Angle par locale sur les pages outsourcing/expertises/talents** : la version **FR** s'adresse à la France, la Belgique et au Canada (consultants francophones certifiés, fuseau quasi identique, TJM compétitifs) ; les versions **AR et EN** s'adressent aux pays du Golfe (EAU, Arabie Saoudite, Qatar : équipes arabophones/anglophones, réactivité, coûts, remote). Même page, argumentaire rédigé nativement pour chaque marché — pas une traduction.

### 7.2 Technique
- `generateMetadata` par page et par locale : title ≤ 60 car. (pattern `%s | Tachfir`), description ≤ 155 car. **rédigée pour le clic** (pas générique), canonical, `alternates.languages` complets (`ar`, `fr`, `en`, `x-default` → `ar`). Pour articles, formations et expertises : title/description depuis les **champs SEO localisés du CMS** ; alternates hreflang générés à partir des slugs localisés du même document.
- **JSON-LD** (composant réutilisable, validé) : `Organization` + `ProfessionalService` (NAP, **`areaServed` : MA, FR, BE, CA, AE, SA, QA**, logo, sameAs LinkedIn) globaux · `Service` sur chaque page service **et chaque expertise** · `Course` (+ `hasCourseInstance`, `provider`) sur chaque formation · `Article` sur le blog · `FAQPage` sur chaque bloc FAQ · `BreadcrumbList` partout.
- `sitemap.ts` multilingue avec alternates hreflang, **alimenté dynamiquement par le CMS** (posts + formations + expertises publiés) · `robots.ts` (Disallow `/admin`, `/api`) · OG images dynamiques par locale via `next/og` (titre AR/FR/EN sur fond de marque).
- Un seul H1 par page, hiérarchie Hn logique, `alt` localisés sur toutes les images (champ obligatoire dans la collection `media`).
- Maillage interne : chaque page service pointe vers devis + expertise(s) liée(s) + formation liée + 1 article de blog lié ; chaque expertise pointe vers la page outsourcing + `/devis` ; chaque article pointe vers 1 service + `/devis`.
...
- **Collections** (tous les champs textuels marqués `localized: true`) :
  ...
  - `expertises` : nom (ex. Java/Spring), slug localisé, accroche, description, profils & séniorités disponibles (junior/confirmé/senior), modalités (régie / forfait / placement), technologies associées (tags), champs SEO, statut. **L'équipe pourra ajouter une nouvelle expertise (Data, SAP, .NET…) sans toucher au code.**
  - `candidatures` — **mini-vivier de talents (ATS léger)** : créées par le formulaire `/talents`, **jamais publiques**. Champs candidat : nom, email, téléphone, expertise (relation), séniorité, disponibilité, remote/mobilité, LinkedIn, CV (upload **privé**, access control strict — jamais servi aux anonymes), consentement. Champs internes : statut (nouveau / qualifié / entretien / placé), notes.
  ...
  - **Global `settings`** : adresse, téléphone, WhatsApp, email, LinkedIn, textes de bandeaux (ex. « Réponse sous 24-48 h », « Shortlist sous 72 h ») — consommé par le footer, la page contact et le JSON-LD.
- **Admin** : interface en **français par défaut** (i18n Payload, activer aussi ar/en). Rôles : `admin` (tout) / `editor` (contenus + consultation du vivier — pas d'accès aux utilisateurs ni aux settings destructifs). Utilisateur admin initial créé par le seed depuis `PAYLOAD_ADMIN_EMAIL` / `PAYLOAD_ADMIN_PASSWORD` (mot de passe à changer à la première connexion — le rappeler dans `TODO-CLIENT.md`).
- **Front** : lecture via la **Local API de Payload dans les Server Components** (pas de fetch REST interne). `generateStaticParams` sur les slugs publiés + hooks `afterChange`/`afterDelete` → revalidation ciblée. Access control : les brouillons ne sont **jamais** servis aux visiteurs anonymes. Live preview Payload pour `posts`, `formations` et `expertises` si l'intégration reste simple, sinon le noter dans `DECISIONS.md`.
- **Script `npm run seed` idempotent** : injecte les 12 articles, 6 formations, 5 expertises, 3 témoignages `[EN_ATTENTE]`, les 5 clients et les settings placeholders (§12).

## 9. Contenus à générer (3 langues — professionnels et vendeurs)

Ton : expert, rassurant, orienté résultats et conformité — crédible face à un DSI, un acheteur public marocain, un décideur français/belge/canadien et un client du Golfe. Chaque locale est **rédigée nativement** (pas traduite littéralement), avec l'angle marché défini en §7.1.
...
- **Page Outsourcing & Talents IT** : les 4 offres (régie/staff augmentation · offshoring-nearshoring · TMA/centre de services · talent hunting) ; processus « brief → shortlist sous 72 h → entretiens → démarrage » en timeline ; arguments FR (francophonie, fuseau, TJM, certifications) et AR/EN (Golfe : arabophone/anglophone, remote, réactivité) ; profils phares avec liens vers les 5 expertises ; CTA « Décrire votre besoin » + « Recevoir des profils sous 72 h ».
- **5 landing pages Expertises seedées dans le CMS** (Java/Spring, Angular/React, ServiceNow, Pentesting, DevOps & Cloud) : structure type — accroche marché, ce que font nos consultants, séniorités, modalités, mini-FAQ, CTA. 300-500 mots par locale, optimisées sur « consultant/développeur {techno} ».
- **Page /talents (candidats)** : pourquoi rejoindre le vivier Tachfir (missions Maroc/France/Golfe, remote, accompagnement), types de profils recherchés, formulaire de dépôt de CV.
- **Page Fourniture informatique & surveillance (spéciale, sans catalogue)** :
  - Message central : « Pas de catalogue en ligne : chaque demande est traitée sur mesure. Envoyez votre besoin, votre bon de commande (سند الطلب) ou votre appel d'offres (طلب العروض), recevez un عرض أثمان / devis sous 24-48 h. »
  - Familles couvertes (texte, pas de fiches produit) : postes de travail & portables, serveurs & stockage, réseau & sécurité, **vidéosurveillance (caméras, enregistreurs NVR) & contrôle d'accès**, impression & scan, licences logicielles, consommables & accessoires.
  - Bloc réassurance secteur public : réponse aux **bons de commande et appels d'offres**, facturation conforme (ICE, RC), délais de livraison engagés, garantie constructeur, SAV, installation sur site, livraison `[ZONES — ex. tout le Maroc]`.
...
- **Blog — 4 sujets × 3 langues = 12 articles seedés dans le CMS en statut publié** (800-1200 mots, SEO on-page complet) : ① « Marchés publics : réussir votre commande de matériel informatique et de vidéosurveillance au Maroc » / سند الطلب ومعدات المراقبة: دليل الإدارات المغربية ② « Nearshore au Maroc : pourquoi les entreprises françaises, belges et canadiennes externalisent leur développement » ③ « Les certifications IT les plus demandées au Maroc » / أهم الشهادات المعلوماتية المطلوبة ④ « Pourquoi les entreprises du Golfe choisissent le Maroc pour le تعهيد IT » / لماذا تختار شركات الخليج المغرب لتعهيد خدماتها التقنية / "Why GCC companies outsource IT to Morocco".
...
- **/devis (formulaire principal)** : type de demande (Développement / Cybersécurité / **Outsourcing & Talents** / Formation / **Fourniture matériel & surveillance**) · raison sociale · secteur (public / privé) · ICE (optionnel) · nom & fonction · email · téléphone · description du besoin · budget estimatif (fourchettes) · délai souhaité · **upload fichier** (BC, CPS, cahier des charges — PDF/images, ≤ 10 Mo, joint à l'email) · case consentement **Loi 09-08**.
- **/talents (dépôt de CV)** : nom · email · téléphone · expertise (liste depuis la collection `expertises` + « Autre ») · séniorité · disponibilité · remote/mobilité · LinkedIn · **CV (PDF ≤ 10 Mo)** · consentement. Soumission → **création d'une entrée `candidatures` dans le CMS** (CV en upload privé) + email de notification à `CONTACT_EMAIL`.
...
- Bandeaux de réassurance près des CTA : « Réponse sous 24-48 h » (devis/fourniture), « Shortlist sous 72 h » (outsourcing/expertises) — textes éditables via le global `settings`.
...
3. **CMS Payload** : installation intégrée, `localization` ar/fr/en, collections (`posts`, `formations`, `expertises`, `candidatures`, `testimonials`, `clients`, `media`) + global `settings`, rôles & access control (CV privés !), admin en français, adapters DB/médias par environnement, squelette du script de seed.
4. **Pages statiques + contenus d'interface** : toutes les routes §6, messages complets AR/FR/EN.
5. **Blog, formations & expertises côté front** : listings et fiches branchés sur le CMS (Local API), filtres, JSON-LD Course/Article/Service, hooks de revalidation, slugs localisés dans le sélecteur de langue.
6. **Formulaires & API** : devis (avec upload), dépôt de CV `/talents` (→ collection `candidatures` + notification), contact, emails, états localisés.
7. **SEO technique** : metadata + hreflang, JSON-LD globaux, sitemap dynamique, robots, OG dynamiques, redirections.
8. **Seed complet des contenus** : 12 articles, 6 formations, 5 expertises, témoignages, clients, settings — puis vérification du rendu réel des pages.
9. **QA finale & rapport** : checklist §14, `DECISIONS.md`, `TODO-CLIENT.md`, `README.md` (installation, guide GitHub → Vercel → Neon → Blob → env → seed, ajout d'un article/formation/expertise via l'admin, consultation du vivier), tag `v1.0.0`.
...
- [ ] Les 3 locales se rendent ; RTL visuellement correct (header, cards, formulaires, breadcrumb) en clair **et** sombre
...
- [ ] Rôle `editor` : peut gérer les contenus et consulter le vivier, mais ne peut modifier ni les utilisateurs ni la configuration ; brouillons invisibles côté public
- [ ] Les 5 landings Expertises se rendent en 3 langues avec JSON-LD `Service` valide
- [ ] `/talents` : dépôt de CV OK → candidature visible dans l'admin ; **le fichier CV est inaccessible publiquement** (tester l'URL directe)
- [ ] Canonical + hreflang (ar/fr/en/x-default) présents sur 100 % des pages, y compris articles, formations et expertises (slugs localisés)
- [ ] JSON-LD valide sur : accueil, 1 service, 1 expertise, 1 formation, 1 article, FAQ
...
- [ ] 404 localisée fonctionnelle ; sélecteur de langue conserve la page (y compris sur un article et une expertise)
